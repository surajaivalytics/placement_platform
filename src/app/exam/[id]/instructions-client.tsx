...
export function MockTestInstructions({ test }: { test: any }) {
    ...
    const handleStart = async () => {
        if (agreed) {
            ...
            // Consider saving start time or session ID here
            router.push(`/exam/${test.id}/test`);
        }
    };

    return (
        <div className="flex-1 bg-gray-50 p-6 md:p-10 flex justify-center min-h-screen">
            <Card className="max-w-4xl w-full shadow-lg border-t-4 border-t-[#181C2E]">
                <div className="p-6 md:p-8 space-y-6">

                    <div className="border-b border-gray-200 pb-4">
                        <h1 className="text-2xl font-bold text-[#181C2E]">{test.title}</h1>
                        <p className="text-gray-500 mt-1">{test.description || "Assessment Guidelines & Instructions"}</p>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div className="bg-blue-50/50 p-4 rounded-lg border border-blue-100 flex flex-col items-center text-center gap-2">
                            ...
                            <p className="text-xs text-gray-600">Total duration is {test.duration} minutes. Timer cannot be paused.</p>
                        </div>
                    ...
                    <div className="space-y-4 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar bg-gray-50 p-4 rounded-lg border border-gray-200">
                        ...
                        <ul className="list-disc pl-5 space-y-2 text-sm text-gray-700">
                            <li>The examination consists of multiple choice and technical questions.</li>
                            <li>Total Questions: {test._count?.questions || 0}.</li>
                    ...
                    <div className="flex justify-between items-center pt-2">
                        <Link href={`/exam/${test.id}/dashboard`}>
                            <Button variant="ghost" className="text-gray-500">Back to Dashboard</Button>
                        </Link>
                        <Button
